...
int loracloud_get_location(char* token, void* writedata, char* postdata)
{
    CURL *curl;
    CURLcode res;
    ...
    char thistoken[128] = {'\0'};
    snprintf(thistoken, sizeof(thistoken), "Ocp-Apim-Subscription-Key: %s", token);

    curl = curl_easy_init();
    if(curl) {
        curl_easy_setopt(curl, CURLOPT_CUSTOMREQUEST, "POST");
        curl_easy_setopt(curl, CURLOPT_URL, "https://gls.loracloud.com/api/v3/solve/singleframe");
        curl_easy_setopt(curl, CURLOPT_FOLLOWLOCATION, 1L);
        curl_easy_setopt(curl, CURLOPT_DEFAULT_PROTOCOL, "https");
        struct curl_slist *headers = NULL;
        headers = curl_slist_append(headers, "Content-Type: application/json");
        headers = curl_slist_append(headers, thistoken);
        curl_easy_setopt(curl, CURLOPT_HTTPHEADER, headers);
        ...
        res = curl_easy_perform(curl);
        if (res != CURLE_OK) {
            MSG_DEBUG(LOG_WARNING, "WARNING~, [loracloud] return: %s\n", strlen(errbuf) > 1 ? errbuf : curl_easy_strerror(res));
        }
    }
    curl_easy_cleanup(curl);
    return (int)res;
