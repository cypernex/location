...
static int parse_serv_cfg(const char * conf_file) {
    ...
    /* point to the location_conf configuration object */
    conf_obj = json_object_get_object(json_value_get_object(root_val), "location_conf");
    if (conf_obj == NULL) {
        MSG_DEBUG(LOG_INFO, "INFO~ %s does not contain a JSON object named location_confs\n", conf_file);
        return -1;
    } else {
        MSG_DEBUG(LOG_INFO, "INFO~ %s does contain a JSON object named location_confs, parsing location parameters\n", conf_file);
    }

    str = json_object_get_string(conf_obj, "loctype");  // default type is indoor
    if (str != NULL) {
        if (!strncmp(str, "indoor", 6)) {
            loccfg.isindoor = true;
        } else {
            loccfg.isindoor = false;
        }
        MSG_DEBUG(LOG_INFO, "INFO~ location type is configured to %s\n", str);
    }

    str = json_object_get_string(conf_obj, "locmap");  // default map is mapwize
    if (str != NULL) {
        if (!strncmp(str, "mapwize", 7)) {
            loccfg.locmap = MAPWIZE;
        } else if (!strncmp(str, "traccar", 7)) {
            loccfg.locmap = TRACCAR;
        } else 
            loccfg.locmap = MAPWIZE;
        MSG_DEBUG(LOG_INFO, "INFO~ location type is configured to %s\n", str);
    }

    /* point to the mqtt_conf configuration object */
    ...
    str = json_object_get_string(conf_obj, "token");
    if (str != NULL) {
        loccfg.token = lgw_strdup(str);
        MSG_DEBUG(LOG_INFO, "INFO~ token is configured to %s\n", loccfg.token);
    }
...
int main(int argc, char* argv[])
{
    ...
    if (loccfg.isindoor && loccfg.locmap == MAPWIZE) {   //indoor and map configure to MAPWIZE

        MSG_DEBUG(LOG_INFO, "DEBUG~ [MAPWIZE] getting placetype...!\n");
	...
	if ((rc = MQTTAsync_create(&client, url, loccfg.clientid, MQTTCLIENT_PERSISTENCE_NONE, NULL))
			!= MQTTASYNC_SUCCESS)
	{
		MSG_DEBUG(LOG_INFO, "DEBUG~ Failed to create client, return code %s\n", MQTTAsync_strerror(rc));
		rc = EXIT_FAILURE;
		goto destroy_exit;
	}
	...
	{
		MSG_DEBUG(LOG_INFO, "DEBUG~ Failed to set callbacks, return code %s\n", MQTTAsync_strerror(rc));
		rc = EXIT_FAILURE;
		goto destroy_exit;
	}
...
static void thread_parse_payload() 
{
    ...
    JSON_Object* b_obj = NULL;  //branch object
    JSON_Array* gw_array = NULL;
    JSON_Value* val = NULL; /* needed to detect the absence of some fields */
    const char *str; /* pointer to sub-strings in the JSON data */
    ...
    char frame_buff[BUFF_SIZE/2];
    char* req_buff;

    int i, j, count;  // i for loop, j for tmp length point, count for count array
    int buff_idx, f_idx, gw_idx; // buff_idx for index buff postion, f_idx for index frame postion, gw_idx for rollback

    bool parse_ok;  // for skip procedure
    ...
    while (!exit_sig && !quit_sig) {
        ...
        if (root_obj == NULL) {
            MSG_DEBUG(LOG_WARNING, "WARNING~  invalid JSON object,  aborted\n");
            lgw_free(payload_entry->content);
            lgw_free(payload_entry);
            continue;
        }
        ...
        switch (loccfg.locmap) {
            case MAPWIZE:
                inode_entry->type = iBEACON;

                switch (payload_entry->type) {
                    case TTN:
                        str = json_object_get_string(root_obj, "dev_id");
                        if (str != NULL) {
                        ...
                        str = json_object_get_string(root_obj, "hardware_serial");
                        if (str != NULL) {
                        ...
                        b_obj = json_object_get_object(root_obj, "payload_fields");
                        if (b_obj == NULL) {
                        ...
                        str = json_object_get_string(b_obj, "UUID");
                        if (str != NULL) {
                        ...
                        val = json_object_get_value(b_obj, "MAJOR");
                        if (str != NULL) {
                        ...
                        val = json_object_get_value(b_obj, "MINOR");
                        if (val != NULL) {
                        ...
                        val = json_object_get_value(b_obj, "RSSI");
                        if (val != NULL) {
            ...
            case TRACCAR:
                switch (payload_entry->type) {
                    case TTN:
                        ...
                        // star gateways body constraction
                        gw_array = json_object_get_array(root_obj, "gateways");
                        if (gw_array != NULL) {
                            count = json_array_get_count(gw_array);
                            req_buff = lgw_malloc(sizeof(char) * BUFF_SIZE);   // req_buff is request body of loracloud api
                            inode_entry->reqbody = req_buff;
                            memset(frame_buff, 0, sizeof(frame_buff));
                            buff_idx = 0; // buff length index
                            f_idx = 0;    // frame length index
                            j = sprintf(req_buff, "{\"gateways\":[");  // {"gateways":[ array ], "frames":[ array ]}
                            buff_idx += j;
                            for (i = 0; i < count; i++) {
                                b_obj = json_array_get_object(gw_array, i);
                                if (b_obj == NULL) continue;
                                gw_idx = 0;
                                str = json_object_get_string(b_obj, "gtw_id");
                                if (NULL == str) continue;
                                // {"gatwayId":[string], "latitude":[float], "longitude":[float], "altitude":[float]}
                                j = sprintf(req_buff + buff_idx, "{\"gatewayId\": \"%s\",", str);  

                                val = json_object_get_value(b_obj, "latitude");
                                if (NULL == str) continue;
                                buff_idx += j;  // the req_buff length
                                gw_idx += j;    // each gateway object length
                                j = sprintf(req_buff + buff_idx, "\"latitude\": \"%f\",", (float)json_value_get_number(val));

                                val = json_object_get_value(b_obj, "longitude");
                                if (NULL == str) {
                                    buff_idx -= gw_idx;   // not a completion gateway object, skip  
                                    continue;
                                }
                                buff_idx += j;
                                gw_idx += j;
                                j = sprintf(req_buff + buff_idx, "\"longitude\": \"%f\",", (float)json_value_get_number(val));
                                
                                val = json_object_get_value(b_obj, "altitude");
                                if (NULL == str) {
                                    buff_idx -= gw_idx;
                                    continue;
                                }
                                buff_idx += j;
                                gw_idx += j;
                                j = sprintf(req_buff + buff_idx, "\"altitude\": \"%s\"},", (float)json_value_get_number(val));

                                // extract frame, format: [ gateway, antenna, tdoa, rssi, snr ]
                                str = json_object_get_string(b_obj, "gtw_id");
                                j = sprintf(frame_buff + f_idx, "[\"%s\",0,", str);  // gatewayid, antenna set to 0(unknown)
                                f_idx += j;

                                val = json_object_get_value(b_obj, "timestamp");        // timestemp for TDOA ?
                                j = sprintf(frame_buff + f_idx, "\"%u\",", (uint32_t)json_value_get_number(val));
                                f_idx += j;

                                val = json_object_get_value(b_obj, "rssi");
                                j = sprintf(frame_buff + f_idx, "%.1f,", (float)json_value_get_number(val));
                                f_idx += j;

                                val = json_object_get_value(b_obj, "snr");
                                j = sprintf(frame_buff + f_idx, "%.1f],", (float)json_value_get_number(val));
                                f_idx += j;
                            }

                            req_buff[buff_idx - 1] = ' ';   // change comma to blank
                            req_buff[buff_idx++] = ']';     
                            req_buff[buff_idx++] = ',';     
                            ...
                            j = sprintf(req_buff + buff_idx, "\"frames\":[%s]}", frame_buff);  // req_buff: { "gateways":[ array ], "frames": [ array ] }
                            
                            buff_idx += j;

                            req_buff[buff_idx] = '\0';

                            MSG_DEBUG(LOG_INFO, "DEBUG~ gateways object: %s\n", req_buff);

                        } else {
                            MSG_DEBUG(LOG_WARNING, "WARNING~ can't get gateways array, skip to next payload\n");
                            parse_ok = false;
                        }
...
static void thread_create_place() 
{
    ...
    char place_id[25] = {0};
    char place_prefix[10];
    char place_data[1024] = {0};

    inode_s* inode_entry = NULL;
    ibeacon_s* ibeacon_entry = NULL;

    lgw_gen_str(place_prefix, sizeof(place_prefix));
    ...
    while (!exit_sig && !quit_sig) {
        ...
        LGW_LIST_TRAVERSE(&ibeacon_list, ibeacon_entry, list) {
            if (inode_entry->minor != ibeacon_entry->minor) {
                ...
                snprintf(place_id, sizeof(place_id), "%s%s", place_prefix, inode_entry->deveui);
...
static void thread_get_gps() 
{
    inode_s* inode_entry = NULL;
    curlstr_s* coordata;

    while (!exit_sig && !quit_sig) {
        lgw_wait_sem(&parse_inode_sem, DEFAULT_LOOP_MS); 
        MSG_DEBUG(LOG_INFO, "DEBUG~  Trigger create place thread...\n");
        LGW_LIST_LOCK(&inode_list);
        LGW_LIST_TRAVERSE_SAFE_BEGIN(&inode_list, inode_entry, list) {
            if (inode_entry) {
        ...
        if (inode_entry == NULL || inode_entry->reqbody == NULL) continue;

        coordata = init_curl_write_data();
        loracloud_get_location(loccfg.token, coordata, inode_entry->reqbody);
    }
}
